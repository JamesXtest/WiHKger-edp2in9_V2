...
def rhrread_process(): #本港地區天氣報告
    ...
    if len(rhrread_data["uvindex"]) == 0:
        print('紫外線指數 : 0')
        print('紫外線強度 : 低')
    else:
        print('紫外線指數 :', rhrread_data["uvindex"]["data"][0]["value"])
        print('紫外線強度 :', rhrread_data["uvindex"]["data"][0]["desc"])
        # print('紫外線資料位置 :', rhrread_data["uvindex"]["data"][0]["place"])
        # print('紫外線資料詳情 :', rhrread_data["uvindex"]["recordDesc"])
...
def fnd_process(): # 九天天氣預報   
    ...
    for i in range (2):       
        ...
        print("預測溫度 :", fnd_data["weatherForecast"][i]["forecastMintemp"]["value"],"-", 
            fnd_data["weatherForecast"][i]["forecastMaxtemp"]["value"], "°")
    ...
    text_W, text_H = draw.textsize("明日預測", font=font_cn18)
    draw.text((int((current_W-text_W)/2+180+yP), 10), "明日預測", font=font_cn15, fill=0)
...
try:
    ...
    while True:
        ...
        Himage.save(os.path.join(tempdir, 'Temp_WiHKger_epd2in9_V2.bmp'))
